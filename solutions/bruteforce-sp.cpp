...
int main(int argc, char const *argv[])
{
    ...
    float answer = HUGE_VAL;
    
    for (auto p : points) {
        for (auto q : points) {
            if (p != q) {
                float delta_x = std::abs(std::get<0>(p)-std::get<0>(q));
                float delta_y = std::abs(std::get<1>(p)-std::get<1>(q));
                float delta_z = std::abs(std::get<2>(p)-std::get<2>(q));
